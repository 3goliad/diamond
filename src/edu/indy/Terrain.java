	...
	public Group populateDiamond() {
		...
		for (int x = 5; x < _size - 5; x += 1) {
			for (int y = 5; y < _size - 5; y += 1) {
				...
				// if (placeValue>.9 && terrainValue.getZ() < 0 &&
				// terrainValue.getZ() > -_scale){
				// //Temporary specific to trees
				// ImageView treeview = tree.getView();
				// double rotate = ThreadLocalRandom.current().nextDouble(-8,
				// 8);
				// treeview.getTransforms().add(new Rotate(rotate,
				// Rotate.Y_AXIS));
